...
class DailyStatsRepository {
  ...
  Future<void> updateActivityStats({required String userId, required DateTime date, double? addEnergy, int? addMinutes, double? distance,}) async {
    final existing = await getDailyStats(userId, date);
    final newStats = DailyStats(
      userId: userId,
      date: date,
      activeEnergyBurned:(existing?.activeEnergyBurned ?? 0) + (addEnergy ?? 0),
