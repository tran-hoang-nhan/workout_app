...
class WorkoutRepository {
  ...
  Future<List<Map<String, dynamic>>> searchWorkouts(String query) async {
    try {
      final response = await _supabase.from('workouts').select().or('title.ilike.*$query*,description.ilike.*$query*')
  ...
  Future<List<int>> getExerciseIdsByMuscleGroupKeywords(
    ...
    try {
      ...
      final orFilter = keywords.map((k) => 'muscle_group.ilike.*${k.trim()}*').join(',');
      final response = await _supabase.from('exercises').select('id').or(orFilter);
  ...
  Future<List<Map<String, dynamic>>> getWorkoutsByTitleKeywords( List<String> keywords,) async {
    try {
      if (keywords.isEmpty) return [];
      ...
      final response = await _supabase.from('workouts').select().or(clauses.join(',')).order('id', ascending: true);
      return (response as List).map((json) => processWorkoutJson(_supabase, json)).toList();
    } catch (e, st) {
      throw handleException(e, st);
    }
